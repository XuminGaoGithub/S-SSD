...
#caffe_root = './'
caffe_root = '/home/mm/ssd-condep/caffe'
os.chdir(caffe_root)
...
##########
        ...
        cv2.putText(image2, 'FPS:', (500,35), cv2.FONT_ITALIC, 1.2, (0, 0, 255), 2)
        cv2.putText(image2, '%d' % (1.0/(time.time()-start_time)), (580,35), cv2.FONT_ITALIC, 1.2, (0, 0, 255), 2)

        cv2.imshow('img',image2)
        print("FPS: ",1.0/(time.time()-start_time))
        if cv2.waitKey(1) & 0xFF == ord('q'):
            cv2.imwrite("11.png",image2)
            #break
    cap.release()
    cv2.destroyAllWindows()
